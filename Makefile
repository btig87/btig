...
pre-build-tests-fast: check-for-non-ascii-urls check-for-wrong-filename-assignments \
    ...
    check-for-consistent-btig-core-titles \
...
## future)
	...
	$S git grep -h 'assign summary_' _data/devdocs/en/btig-core/rpcs/rpcs/ | LANG=UTF-8 sort >> _includes/helpers/summaries.md
...
## locally
	...
	$S curl -s -o- https://btig.org/sha256sums.txt \
	  | sort - _site/sha256sums.txt \
	  | uniq -u \
	  | sort -k2

check-for-broken-btig-core-download-links:
## Ensure that the links from the Download page to the current btig
## Core binaries are correct
	$S grep 'class="[^"]*dl[^"]*"' _site/en/download.html \
	  | sed 's/.*href="//; s/".*//' \
	  | while read url ; do \
	    if [ "$${url##http*}" ]; then \
	      curl -sI "https://btig.org$$url" ; \
	    else \
	      curl -sI "$$url" ; \
...
check-for-consistent-btig-core-titles:
## Ensure all page titles in the en/btig-core/ hierarchy mention
## btig Core
	$S grep -r -L '^title:.*btig Core' en/btig-core/ | eval $(ERROR_ON_OUTPUT)
